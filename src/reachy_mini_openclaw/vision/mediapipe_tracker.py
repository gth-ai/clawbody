...
# MediaPipe dropped the legacy Solutions API (mp.solutions) — recent releases
# expose only the Tasks API, which this tracker does not implement. Fail here
# rather than at first frame, so get_head_tracker() falls through to YOLO.
if not hasattr(mp, "solutions"):
    raise ImportError(
        f"mediapipe {getattr(mp, '__version__', '?')} no longer ships the "
        "mp.solutions API this tracker needs. Use the YOLO tracker "
        "(HEAD_TRACKER_TYPE=yolo, pip install ultralytics supervision), or pin "
        "an older mediapipe that still provides mp.solutions."
    )
