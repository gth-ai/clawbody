...
class ClawBodyCore:
    ...
    def __init__(
        ...
        enable_face_tracking: Optional[bool] = None,
        ...
        Args:
            ...
            enable_face_tracking: Override config.ENABLE_FACE_TRACKING
            head_tracker_type: Override config.HEAD_TRACKER_TYPE
                ("yolo", "mediapipe", or None to auto-detect)
        """
        ...
        if robot is not None:
            ...
            robot_kwargs = {
                "connection_mode": config.REACHY_CONNECTION_MODE,
                "media_backend": config.REACHY_MEDIA_BACKEND,
            }
            if config.REACHY_HOST:
                robot_kwargs["host"] = config.REACHY_HOST
            if robot_name:
                robot_kwargs["robot_name"] = robot_name
            logger.info(
                "Robot connection: host=%s mode=%s media=%s",
                robot_kwargs.get("host"),
                robot_kwargs["connection_mode"],
                robot_kwargs["media_backend"],
            )


            # Raise rather than sys.exit: this constructor also runs inside the
            # Gradio UI thread, where SystemExit is not an Exception, escapes
            # the handler's `except Exception`, and leaves the Start button
            # failing with nothing useful on screen. main() turns it into an
            ...
            try:
                ...
                raise RuntimeError(
                    f"Could not reach the robot at {robot_kwargs.get('host')}: {e}"
                ) from e
        ...
        if enable_camera:
            ...
            # Explicit arguments win over config, so the Gradio UI and the CLI
            # flags can both steer this.
            want_tracking = (
                config.ENABLE_FACE_TRACKING
                if enable_face_tracking is None
                else enable_face_tracking
            )
            if want_tracking:
                self.head_tracker = self._initialize_head_tracker(
                    head_tracker_type or config.HEAD_TRACKER_TYPE
                )
    ...
    def _initialize_head_tracker(self, tracker_type: Optional[str] = None) -> Optional[Any]:
        ...
        if tracker_type == "yolo":
            try:
                ...
                # Not "fast enough": detection runs inside the tracking control
                # loop, and on this machine CPU costs 27.6 ms/frame against
                # 6.7 ms on the GPU, for identical detections. VISION_DEVICE
                # defaults to "auto", which picks the local accelerator.
        ...
        while not self._should_stop():
            ...
            if output is not None:
                if isinstance(output, tuple):
                    ...
                    # Tell the handler how much speech the robot still has to
                    # play, so echo suppression tracks actual playback rather
                    # than the moment the audio was queued.
                    chunk_seconds = len(audio_data) / output_sr
                    self.handler.note_speaker_audio(chunk_seconds)

                    # Silence here is ambiguous: no reply, or a reply that was
                    # discarded before playback. Report what actually reached
                    # the speaker, at most once a second.
                    pushed_chunks += 1
                    pushed_seconds += chunk_seconds
                    now = time.monotonic()
                    if now - last_report >= 1.0:
                        last_report = now
                        logger.info(
                            "Audio out: %d chunks, %.1fs pushed to speaker",
                            pushed_chunks,
                            pushed_seconds,
                        )
...
def main() -> None:
    ...
    if args.gradio:
        ...
        launch_gradio(
            ...
            # Without these, --no-face-tracking is silently ignored in UI mode
            # and the tracker falls back to auto-detection.
            enable_face_tracking=config.ENABLE_FACE_TRACKING and not args.no_face_tracking,
