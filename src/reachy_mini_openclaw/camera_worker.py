...
class CameraWorker:
    ...
    def __init__(self, reachy_mini: ReachyMini, head_tracker: Any = None) -> None:
        ...
        # Target period of the tracking loop. The loop compensates for
        # detector time instead of sleeping a fixed amount on top of it.
        # The ceiling is the detector cost (~11 ms on GPU, ~28 ms on CPU) plus
        # the frame fetch, which blocks on the camera's own 30 fps.
        self.loop_period = 1.0 / config.FACE_TRACKING_HZ

        # Smoothing factor for the exponential moving average.
        # Derived from the loop rate rather than hard-coded: a fixed alpha
        # silently changes how fast the head converges whenever the rate
        # changes, which is exactly the bug that made tracking feel sluggish.
        # Holding the 95%-convergence time constant keeps the feel identical
        # at any rate. 0.42 s reproduces the original alpha=0.25 at 25 Hz.
        self.convergence_95_seconds = 0.42
        steps = max(1.0, self.convergence_95_seconds * config.FACE_TRACKING_HZ)
        self.smoothing_alpha = 1.0 - 0.05 ** (1.0 / steps)
        self._loop_count = 0
        self._rate_report_time = time.time()
    ...
    def _working_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                ...
                # Hold ~25 Hz by sleeping only the remainder of the period.
                # A flat sleep(0.04) added itself to the detector's own cost
                # (~31 ms for YOLO on a 720p frame), which put the real loop
                # near 13 Hz — and the EMA smoothing below is tuned for 25 Hz,
                # so tracking converged about twice too slowly.
                self._loop_count += 1
                now = time.time()
                ...
                time.sleep(max(0.0, self.loop_period - elapsed))
